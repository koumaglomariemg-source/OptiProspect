...
export const api = {
  ...
  publicProspectInfo: (id, token) =>
    request(
      `/public/prospects/${id}/info?token=${encodeURIComponent(token || "")}`,
    ),
  publicRespond: (id, data) =>
    request(`/public/prospects/${id}/respond`, {
      method: "POST",
  ...
  markRelanceDone: (id) =>
    request(`/prospects/${id}/relance-done`, { method: "POST" }),
  steps: (prospectId) => request(`/prospects/${prospectId}/steps`),
  saveStep: (progressId, data) =>
    request(`/steps/${progressId}`, {
      method: "PUT",
      body: JSON.stringify({ data }),
    }),
  validateStep: (progressId) =>
    request(`/steps/${progressId}/validate`, { method: "POST" }),
  unvalidateStep: (progressId) =>
    request(`/steps/${progressId}/unvalidate`, { method: "POST" }),

  pipelineTemplates: () => request("/pipeline-templates"),
  createPipelineTemplate: (data) =>
    request("/pipeline-templates", {
      method: "POST",
      body: JSON.stringify(data),
    }),
  updatePipelineTemplate: (id, data) =>
    request(`/pipeline-templates/${id}`, {
      method: "PUT",
      body: JSON.stringify(data),
    }),
  setDefaultPipelineTemplate: (id) =>
    request(`/pipeline-templates/${id}/default`, { method: "POST" }),
  deletePipelineTemplate: (id) =>
    request(`/pipeline-templates/${id}`, { method: "DELETE" }),
  ...
  sendMessage: (id, data) =>
    request(`/prospects/${id}/send-message`, {
      method: "POST",
      body: JSON.stringify(data),
    }),
  importProspects: (file) => {
    ...
    return request("/prospects/import", {
      method: "POST",
      body: formData,
      headers: {},
    });
  ...
  interactions: (prospectId) =>
    request(`/prospects/${prospectId}/interactions`),
  addInteraction: (prospectId, data) =>
    request(`/prospects/${prospectId}/interactions`, {
      method: "POST",
      body: JSON.stringify(data),
    }),
  ...
  statsOverview: (params = {}) =>
    request("/stats/overview?" + new URLSearchParams(params)),
  statsByUser: (params = {}) =>
    request("/stats/by-user?" + new URLSearchParams(params)),
  statsTimeline: (days = 30, params = {}) =>
    request(`/stats/timeline?days=${days}&` + new URLSearchParams(params)),
  statsForecast: (params = {}) =>
    request("/stats/forecast?" + new URLSearchParams(params)),
  statsTargets: (yearMonth, params = {}) =>
    request(
      `/stats/targets?year_month=${yearMonth}&` + new URLSearchParams(params),
    ),
  statsCounts: () => request("/stats/counts"),
  statsProspection: (days = 30, params = {}) =>
    request(`/stats/prospection?days=${days}&` + new URLSearchParams(params)),
  statsAtRisk: (params = {}) =>
    request("/stats/at-risk?" + new URLSearchParams(params)),
  statsAging: (params = {}) =>
    request("/stats/aging?" + new URLSearchParams(params)),
  clients: (params = {}) =>
    request("/stats/clients?" + new URLSearchParams(params)),

  setTarget: (userId, yearMonth, targetValue) =>
    request(`/users/${userId}/target`, {
      method: "PUT",
      body: JSON.stringify({
  ...
  deleteTarget: (userId, yearMonth) =>
    request(`/users/${userId}/target/${yearMonth}`, { method: "DELETE" }),

  devis: (params = {}) => request("/devis?" + new URLSearchParams(params)),
  createDevis: (data) =>
    request("/devis", { method: "POST", body: JSON.stringify(data) }),
  updateDevis: (id, data) =>
    request(`/devis/${id}`, { method: "PATCH", body: JSON.stringify(data) }),
  submitDevis: (id) => request(`/devis/${id}/submit`, { method: "POST" }),
  validateDevis: (id, comment = "") =>
    request(`/devis/${id}/validate`, {
      method: "POST",
      body: JSON.stringify({ comment }),
    }),
  refuseDevis: (id, comment = "") =>
    request(`/devis/${id}/refuse`, {
      method: "POST",
      body: JSON.stringify({ comment }),
    }),
