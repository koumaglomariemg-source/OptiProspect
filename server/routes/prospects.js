...
router.get("/", ah(async (req, res) => {
  ...
  if (hasPagination) {
    ...
    const total = (
      await db.get(`SELECT COUNT(*) AS n FROM prospects p${whereSql}`, ...params)
    ).n;
    const rows = await db.all(
      `${base}${whereSql} ORDER BY ${order} LIMIT ? OFFSET ?`,
      ...params,
      l,
      (p - 1) * l,
    );
  ...
  const rows = await db.all(`${base}${whereSql} ORDER BY ${order}`, ...params);
...
router.post("/", ah(async (req, res) => {
  try {
    ...
    if (!b.product?.trim())
      return res.status(400).json({ error: "Le produit à proposer est requis" });
    ...
    const fullName = b.name?.trim() || [b.first_name, b.last_name].filter(Boolean).join(" ").trim();
...
router.get("/:id/events", ah(async (req, res) => {
  const p = await findScopedProspect(req, Number(req.params.id));
  if (!p) return res.status(404).json({ error: "Prospect introuvable" });
  const rows = await db.all(
...
router.patch("/:id", ah(async (req, res) => {
  ...
  for (const f of FIELDS) {
    ...
    if (f === "stage") {
      ...
      v = req.body[f] === "" || req.body[f] === null ? null : num(req.body[f]);
      updates.push(`${f} = ?`);
      params.push(v);
      ...
      if (v) {
        ...
        if (Number.isNaN(d.getTime()) || d < now) {
          return res
            .status(400)
            .json({ error: "La date et l'heure de relance ne peuvent pas être dans le passé" });
        }
        v = `${raw}:00`;
      }
    ...
    const oldS =
      f === "value" || f === "latitude" || f === "longitude"
        ...
        : str(oldV);
    const newS =
      f === "value" || f === "latitude" || f === "longitude" ? num(v) : str(v);
...
router.post("/:id/relance-done", ah(async (req, res) => {
  const id = Number(req.params.id);
  const prospect = await findScopedProspect(req, id);
  if (!prospect) return res.status(404).json({ error: "Prospect introuvable" });
  ...
  await logEvent(id, req.user, "action", "next_action", prospect.next_action, null);
...
router.get("/:id/suggest-message", ah(async (req, res) => {
  const p = await findScopedProspect(req, Number(req.params.id));
  if (!p) return res.status(404).json({ error: "Prospect introuvable" });
  const firstName = (p.name || "").split(" ")[0] || p.name;
  const company = p.company || "votre entreprise";
  const senderFirst = (req.user?.name || "L'équipe OptiProspect").split(" ")[0];
  ...
  let cta;
  if (temp === "chaud") {
    cta = "Seriez-vous disponible pour un échange de 15 minutes cette semaine afin d'aller de l'avant ?";
  } else if (temp === "froid") {
    cta = "Seriez-vous ouvert(e) à un premier échange rapide de 15 minutes pour que je vous en dise plus ?";
  } else {
    cta = "Auriez-vous un moment cette semaine pour un rapide échange de 15 minutes ?";
  }
...
router.post("/:id/send-message", ah(async (req, res) => {
  const p = await findScopedProspect(req, Number(req.params.id));
  if (!p) return res.status(404).json({ error: "Prospect introuvable" });
  if (req.user.role === "manager")
    return res.status(403).json({ error: "Un manager ne peut pas envoyer de message" });
  const { channel, subject, content } = req.body || {};
  const ch = ["email", "whatsapp", "linkedin"].includes(channel) ? channel : "email";
  if (!content?.trim())
  ...
  if (ch === "email") {
    if (!isMailConfigured()) {
      ...
      try {
        ...
        return res.status(500).json({ error: `Échec de l'envoi SMTP : ${e.message}` });
      }
    }
  } else {
    result.skipped = true;
